 ...
 * user input as a Galactic value and convert it to the Roman values
 *
 */
		...
		if (strings.length > 1) {
			...
			if (!string.isEmpty()) {
				...
				while (aTokenizer.hasMoreElements()) {
					...
					Object anAssignedValue = itsDictionary
							.getAssignedValue(anIntergalacticName);
				...
				if (aRomanValues.isEmpty()) {
					System.out.println(
							"Invalid Galatic Values.Please try again with the assigned galactic values");
					return null;
				}
		...
		for (int i = 1; i < a.size(); i++) {
			if (a.get(i) instanceof String) {
				...
				if (count == 2
					...
					System.out.println("Consective Numbers of " + a.get(i)
							+ " are not allowed");
					return true;
				}
