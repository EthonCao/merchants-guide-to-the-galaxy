	 ...
	 public Double converter(List<Object> aRomanValueList) {
		...
		for (int i = 1; i < aRomanValueList.size(); i++) {
			...
			if (aFirstObjectValue instanceof String) {
				...
				RomanNumeral aSecondNumerial = getRomanNumerialValue(
						aFirstValue);
				...
				if (aTempValue.getValue() >= aSecondNumerial.getValue()) {
					...
					if (!isSubtractable(aSecondNumerial.name(),
							aTempValue.name())) {
						return (double) 0;
					}

					aResult += (aSecondNumerial.getValue()
							- aTempValue.getValue());
					...
					if (i < aRomanValueList.size()) {
						if (aRomanValueList.get(i) instanceof String) {
							String aNewTempElement = (String) aRomanValueList
									.get(i);
							RomanNumeral aNewTempNumerial = getRomanNumerialValue(
									aNewTempElement);
		...
		switch (toValue) {
			...
			case "I" :
				if (fromValue.equalsIgnoreCase(RomanNumeral.V.name())
					...
					System.out.println("I cannot be subtracted from '"
							+ fromValue
			...
			case "X" :
				if (fromValue.equalsIgnoreCase(RomanNumeral.L.name())
					...
					System.out.println("'X' cannot be subtracted from '"
							+ fromValue
			...
			case "C" :
				if (fromValue.equalsIgnoreCase(RomanNumeral.D.name())
					...
					System.out.println("'C' cannot be subtracted from '"
							+ fromValue
